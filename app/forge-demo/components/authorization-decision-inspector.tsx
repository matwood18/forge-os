// app/forge-demo/components/authorization-decision-inspector.tsx
import type {
  AuthorizationDecisionInspector,
  AuthorizationDecisionInspectorItem,
} from "@/lib/demo/authorization";

type AuthorizationDecisionInspectorProps = {
  inspector: AuthorizationDecisionInspector;
};

function formatValue(value: string): string {
  return value
    .replace(/_/g, " ")
    .replace(/\b\w/g, (character) => character.toUpperCase());
}

function outcomeStyles(
  outcome: AuthorizationDecisionInspectorItem["outcome"]
): string {
  if (outcome === "authorized") {
    return "border-emerald-900/70 bg-emerald-950/40 text-emerald-300";
  }

  if (outcome === "denied") {
    return "border-rose-900/70 bg-rose-950/40 text-rose-300";
  }

  if (outcome === "requires_human_review") {
    return "border-amber-900/70 bg-amber-950/40 text-amber-300";
  }

  return "border-slate-700 bg-slate-900 text-slate-400";
}

export function AuthorizationDecisionInspectorView({
  inspector,
}: AuthorizationDecisionInspectorProps) {
  return (
    <section className="mb-8 rounded-2xl border border-slate-800 bg-slate-950/70 p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">
            Authorization Decision Inspector
          </h2>

          <p className="mt-1 text-sm text-slate-400">
            Durable authority decisions evaluating whether recommendations may
            proceed toward future action.
          </p>
        </div>

        <div className="rounded-full border border-slate-800 px-3 py-1 text-xs font-medium text-slate-400">
          {inspector.items.length}{" "}
          {inspector.items.length === 1 ? "decision" : "decisions"}
        </div>
      </div>

      <div className="mt-4 space-y-3">
        {inspector.items.length > 0 ? (
          inspector.items.map((item) => (
            <article
              key={item.id}
              className="rounded-xl border border-slate-800 bg-slate-900/70 p-4"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold text-slate-100">
                    {formatValue(item.outcome)}
                  </h3>

                  <p className="mt-1 text-xs text-slate-500">
                    Authority: {formatValue(item.authority)} ·{" "}
                    {item.createdAt.toLocaleTimeString()}
                  </p>
                </div>

                <span
                  className={`rounded-full border px-3 py-1 text-xs font-medium ${outcomeStyles(
                    item.outcome
                  )}`}
                >
                  {formatValue(item.outcome)}
                </span>
              </div>

              <p className="mt-3 text-sm text-slate-400">
                {item.rationale}
              </p>

              <p className="mt-3 text-xs text-slate-500">
                Recommendation: {item.recommendationId}
              </p>

              <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2">
                <p className="text-xs font-medium text-slate-400">
                  Applied policies
                </p>

                {item.appliedPolicyIds.length > 0 ? (
                  <ul className="mt-2 space-y-1">
                    {item.appliedPolicyIds.map((policyId) => (
                      <li
                        key={policyId}
                        className="text-xs text-slate-500"
                      >
                        {policyId}
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="mt-2 text-xs text-slate-600">
                    No policy identifiers recorded.
                  </p>
                )}
              </div>
            </article>
          ))
        ) : (
          <p className="rounded-xl border border-dashed border-slate-800 bg-slate-900/40 p-4 text-sm text-slate-500">
            No authorization decisions were recorded.
          </p>
        )}
      </div>
    </section>
  );
}