...
const resolved = repository.update({
  id: created.id,
  status: "resolved",
...
assert(repository.listByStatus("resolved").length === 1, "resolved concerns remain queryable");
assert(repository.list().length === 1, "repository preserves concern history");
  ...
  JSON.stringify(
    {
      ...
      resolvedConcernStillStored: repository.listByStatus("resolved").length === 1,
    },
    null,
    2
  )
