...
executiveConcernRepository.clear?.();
...
const firstResult = executiveConcernCoordinator.coordinate({
...
const secondResult = executiveConcernCoordinator.coordinate({
  projection: secondProjection,
});

const concern = executiveConcernRepository.findById(
...
assert(executiveConcernRepository.list().length === 1, "runtime should retain one concern");
  ...
  JSON.stringify(
    {
      ...
      concernCount: executiveConcernRepository.list().length,
