...
const createdResult = coordinator.coordinate({
...
assert(repository.list().length === 1, "repository should contain created concern");

const duplicateResult = coordinator.coordinate({
  projection: firstProjection,
});
...
const updatedResult = coordinator.coordinate({
...
assert(repository.list().length === 1, "repository should still contain one concern");

const concern = repository.findById("concern:contact-insurance");
  ...
  JSON.stringify(
    {
      ...
      repositoryConcernCount: repository.list().length,
